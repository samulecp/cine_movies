...
@section('content_header')
    <h1 class="text-sm sm:text-base md:text-lg lg:text-xl">Editar Cajero</h1>
@stop
...
<div class="responsive-form-container">
    ...
    <form action="{{ route('cajero.update', $usuario->cajero->id) }}" method="POST" class="needs-validation" novalidate>
        @csrf
        @method('PUT')
        ...
        <div class="row g-3">
            ...
            <div class="col-12 col-md-6">
                <label for="HoraEntrada" class="form-label">
                    <span class="text-danger">*</span> Hora de Entrada
                </label>
                <input 
                    type="time" 
                    name="HoraEntrada" 
                    ...
                    value="{{ old('HoraEntrada', $usuario->cajero->HoraEntrada ?? '') }}"
                    required
                >
                @error('HoraEntrada')
                    <div class="invalid-feedback d-block">{{ $message }}</div>
            ...
            <div class="col-12 col-md-6">
                <label for="HoraSalida" class="form-label">
                    <span class="text-danger">*</span> Hora de Salida
                </label>
                <input 
                    type="time" 
                    name="HoraSalida" 
                    ...
                    value="{{ old('HoraSalida', $usuario->cajero->HoraSalida ?? '') }}"
                    required
                >
                @error('HoraSalida')
                    <div class="invalid-feedback d-block">{{ $message }}</div>
            ...
            <div class="col-12">
                <label for="Idpuesto" class="form-label">
                    <span class="text-danger">*</span> Puesto
                </label>
                <select 
                    name="Idpuesto" 
                    id="Idpuesto"
                    class="form-select @error('Idpuesto') is-invalid @enderror" 
                    required
                >
                    ...
                    @foreach ($puestos as $puesto)
                        <option 
                            value="{{ $puesto->id }}" 
                            {{ old('Idpuesto', optional($usuario->cajero)->Idpuesto) == $puesto->id ? 'selected' : '' }}
                        >
                            {{ $puesto->nombre }}
                        </option>
                    @endforeach
        ...
        <div class="row g-2 mt-4">
            <div class="col-12 col-sm-auto">
                <a href="{{ route('cajero.index') }}" class="btn btn-secondary w-100 w-sm-auto">
